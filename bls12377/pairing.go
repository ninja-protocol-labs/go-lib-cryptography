...
// using RFC 9380's hash-to-curve with the simplified SWU map (BN254 uses
// SVDW instead; BLS12-377's curve shape admits SSWU) and
...
func HashToG1(msg, dst []byte) (G1Point, error) {
	p, err := bls12377.HashToG1(msg, dst)
	if err != nil {
...
func HashToG2(msg, dst []byte) (G2Point, error) {
	p, err := bls12377.HashToG2(msg, dst)
	if err != nil {
...
// SSWU map, rather than HashToG1's two. It is cheaper but its output is
...
func EncodeToG1(msg, dst []byte) (G1Point, error) {
	p, err := bls12377.EncodeToG1(msg, dst)
	if err != nil {
		return G1Point{}, ErrHashToCurveFailed
	}
	return G1Point{p: p}, nil
...
func EncodeToG2(msg, dst []byte) (G2Point, error) {
	p, err := bls12377.EncodeToG2(msg, dst)
	if err != nil {
		return G2Point{}, ErrHashToCurveFailed
	}
	return G2Point{p: p}, nil
...
func MillerLoop(q G2Point, p G1Point) (GT, error) {
	f, err := bls12377.MillerLoop([]bls12377.G1Affine{p.p}, []bls12377.G2Affine{q.p})
	if err != nil {
		return GT{}, ErrPairingFailed
	}
	return GT{e: f}, nil
...
func MillerLoopN(qs []G2Point, ps []G1Point) (GT, error) {
	...
	f, err := bls12377.MillerLoop(gps, gqs)
	if err != nil {
		return GT{}, ErrPairingFailed
	}
	return GT{e: f}, nil
...
func PairingCheck(qs []G2Point, ps []G1Point) (bool, error) {
	...
	gps, gqs := unwrapPoints(ps, qs)
	ok, err := bls12377.PairingCheck(gps, gqs)
	if err != nil {
...
type PrecomputedLines struct {
	// One entry shorter than BLS12-377's ate loop counter: gnark-crypto
	// drops the final iteration here, where BN254's loop keeps it.
	lines [2][len(bls12377.LoopCounter) - 1]bls12377.LineEvaluationAff
}

// PrecomputeLines computes the line evaluations for q.
func PrecomputeLines(q G2Point) PrecomputedLines {
	return PrecomputedLines{lines: bls12377.PrecomputeLines(q.p)}
}
...
func MillerLoopLines(lines PrecomputedLines, p G1Point) (GT, error) {
	f, err := bls12377.MillerLoopFixedQ(
		[]bls12377.G1Affine{p.p},
		[][2][len(bls12377.LoopCounter) - 1]bls12377.LineEvaluationAff{lines.lines},
	)
	if err != nil {
		return GT{}, ErrPairingFailed
	}
...
func unwrapPoints(ps []G1Point, qs []G2Point) ([]bls12377.G1Affine, []bls12377.G2Affine) {
	gps := make([]bls12377.G1Affine, len(ps))
	gqs := make([]bls12377.G2Affine, len(qs))
	for i := range ps {
		gps[i] = ps[i].p
		gqs[i] = qs[i].p
	}
