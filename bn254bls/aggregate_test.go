...
import (
	...
	"github.com/ninja-protocol-labs/go-lib-cryptography/bn254"
...
func TestRogueKeyAttackSucceedsUnderTheBasicScheme(t *testing.T) {
	...
	sum := sumHonest.Bytes()
	sumPoint, err := bn254.G1PointFromCompressed(sum[:])
	require.NoError(t, err)

	rogue := bn254.G1Generator().Mul(x).Add(sumPoint.Neg()).Bytes()
