...
func TestPublicKeyMinPkFromBytesRejectsInvalid(t *testing.T) {
	tests := []struct {
		...
		// has no matching y: 4³+3 = 67 is a quadratic non-residue mod p.
		// (G1's cofactor is 1, so on-curve and in-subgroup are the same
		// condition here — the only way to be rejected is to miss the
		// curve entirely.)
		{"not on the curve", func() []byte {
			b := make([]byte, PubkeyMinPkLen)
			b[0] = 0x80 // compressed, y even
