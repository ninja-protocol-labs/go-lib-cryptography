...
import (
	...
	"github.com/ninja-protocol-labs/go-lib-cryptography/bls12377"
...
func TestRogueKeyAttackSucceedsUnderTheBasicScheme(t *testing.T) {
	...
	sum := sumHonest.Bytes()
	sumPoint, err := bls12377.G1PointFromCompressed(sum[:])
	require.NoError(t, err)

	rogue := bls12377.G1Generator().Mul(x).Add(sumPoint.Neg()).Bytes()
