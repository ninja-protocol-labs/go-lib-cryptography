// Package bn254edwards implements EdDSA over the twisted Edwards curve
// embedded in BN254's scalar field 𝔽r — the curve known as Baby Jubjub,
// standardised as EIP-2494 and also called BN254's companion curve.
//
// It is not BN254. Baby Jubjub is a separate curve that takes BN254's
// scalar field as its own base field, which is exactly why it exists: a
// zk-SNARK circuit over BN254 computes natively in 𝔽r, so Baby Jubjub
// arithmetic is cheap to prove there while BN254's own 𝔽p arithmetic is
// not. Verifying a signature inside a circuit is the whole point; outside
// one, use ed25519 instead, which is standardised, interoperable and
// faster.
//
...
//     hash.Hash, and both fail without one. See bn254mimc.
