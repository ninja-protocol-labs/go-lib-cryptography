...
func SignSchnorr(priv *PrivateKey, msg []byte) ([SignatureSchnorrLen]byte, error) {
	sig, ok := internal.SchnorrSign(msg, &priv.key)
	if !ok {
		return [SignatureSchnorrLen]byte{}, ErrSigningFailed
	}
	return sig, nil
}
...
func SignSchnorrHedged(priv *PrivateKey, msg []byte, auxRand *[32]byte) ([SignatureSchnorrLen]byte, error) {
...
func VerifySchnorr(pub *PublicKey, msg, sig []byte) bool {
	if len(sig) != SignatureSchnorrLen {
		return false
	}
	...
	var fixed [SignatureSchnorrLen]byte
	copy(fixed[:], sig)
