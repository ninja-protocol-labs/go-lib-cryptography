...
// against each of them rather than only the 256-bit one. The upstream
// vectors record 64 bytes of output, and every BLAKE3 length is a prefix
// of the same stream, so both lengths come from the one entry.
func TestDigest(t *testing.T) {
	in := vectorInput(64)
	want := vectorHashHex(t, 64)

	t.Run("BLAKE3-256", func(t *testing.T) { checkDigest256(t, in, want[:2*Size256]) })
	t.Run("BLAKE3-512", func(t *testing.T) { checkDigest512(t, in, want[:2*Size512]) })
}

// vectorHashHex returns the recorded unkeyed output for the input of the
// given length.
func vectorHashHex(t *testing.T, inputLen int) string {
	t.Helper()
	for _, c := range vectorCases {
		if c.inputLen == inputLen {
			return c.hash
		}
	}
	t.Fatalf("no vector for an input of %d bytes", inputLen)
	return ""
}
